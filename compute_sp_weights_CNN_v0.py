...
Email: liush@ethz.ch
devel log: v22a.py
Description: use CNN to compute Lcm, Lmat, Luu(sparse weigths) from flow *.mat files and optimize CNN
    by comparing with ground truth sparse weights(computed by <compute_sp_weights.py>)
"""
...
class Net_v02(th.nn.Module):
    ...
    def forward(self, img_trimap, mask, sample, x0, steps, N):
        ...
        # Asys = sp.spadd(Lcm_sp, sp.spadd(Lmat_sp, sp.spadd(sp.spadd(KU, known), Luu_sp)))
        Asys = sp.spadd(Lmat_sp, sp.spadd(KU, known))
        b = sp.spmv(sp.spadd(KU, known), kToU)
...
for file_id in range(3,4):
    ...
    for epoch in range(1000):
        ...
        pred = net(img_trimap, weight_mask, sample, x0, steps = 30, N = N)
        ...
        for _ in range(3):
            ...
            print('shape of net.masked_weight', net.masked_weight.cpu().data.numpy().shape)
            x0, err = validation_v3(sample, x0, alpha, param = net.masked_weight.cpu().data.numpy(), valid_steps = 100)
            aux_x0.append(x0.cpu().data.numpy().astype(np.float64))
            aux_err.append(err)
