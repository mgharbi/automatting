...
class Net_v02(th.nn.Module):
    ...
    def forward(self, Atri, Lcm, Lmat, Luu, b, x0, steps):
        ...
        Atri = Variable(th.from_numpy(Atri.astype(np.float64)).cuda())
        Lcm = th.mul(Variable(th.from_numpy(Lcm.astype(np.float64)).cuda()), self.x1)
        Lmat = th.mul(Variable(th.from_numpy(Lmat.astype(np.float64)).cuda()), self.x2)
        diag = th.diag(self.x3)
        Luu = th.matmul(th.matmul(diag, Variable(th.from_numpy(Luu.astype(np.float64)).cuda())), diag)

        b = Variable(th.from_numpy(b.astype(np.float64)).cuda())
...
for epoch in range(100000):
    ...
    x0 = Variable(th.from_numpy(pred.cpu().data.numpy().astype(np.float64)).cuda(), requires_grad=False) # avoid endless graph, which leads to memory explosion
